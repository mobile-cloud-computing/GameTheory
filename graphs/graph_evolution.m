...
for p = 1 : G.P
    ...
    labels = cellstr('');
    for s = 1 : G.S(p)
        plot(G.T, G.X(:, (p-1)*n + s), 'color', cc(s, :),LineWidth=5);
	labels(s) = cellstr(['Coallition Participated ',num2str(strategies(p,s).label)]);
    end
    ylim([0 1])
    xlim([0 G.T(end)])
    hold off

    title_i = ['Evolution of the ', num2str(p), '-th Drone Decision'];
    title(title_i)
